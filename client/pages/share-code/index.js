...
class ShareCodePage extends HTMLElement {
    connectedCallback() {
        this.render();
        state_1.state.setRtdbMyValues();
        state_1.state.subscribe(() => {
            const { rivalName } = state_1.state.getState();
            if (rivalName != "" && window.location.pathname == "/sharecode") {
                router_1.Router.go("/instr");
            }
            this.render();
        });
    }
    ...
    <div class="info">
      ...
      <div class="points">
        <div class="user-box">
          <div class="user-room">${state_1.state.data.myName}: ${state_1.state.data.history.me} </div>
        </div>
      </div>
      <div class="room">
